...
class TestPlannerAgent(unittest.TestCase):
    ...
    def test_planner_and_verifier_cases_default_model(self) -> None:
        ...
        planner_agent_context = AgentContext(
            agent_name="PlannerAgent",
            agent_persona="Helpful planner.",
        )

        plan_verifier_agent_context = AgentContext(
            agent_name="PlanVerifierAgent",
            agent_persona="Plan verifier agent.",
        )

        # Setup the Simple Agent
        agent = PlannerAgentLogic(model=llm, context=planner_agent_context)
        verifier = PlanVerifierAgentLogic(
            model=llm, context=plan_verifier_agent_context
        )

        # Execute the agent
        planner_short_memory = ShortMemory()
        plan_verifier_short_memory = ShortMemory()

        instruction_set = [
            # "What is the difference between BMW X3 and X4?",
            # """Calculate the resistance of copper wire having a length of 1 km
            # and diameter 0.5 mm. (Resistivity of copper 1.7e-8Ωm)""",
            "Calculate the volume of a sphere with radius 5 cm.",
            # """Does electric range of Tesla Model 3 allow for weekly
            # commute between San Francisco and Los Angeles?""",
        ]

        for instruction in instruction_set:
            result = agent.run(instruction, planner_short_memory)
            ...
            verifier_result = verifier.run(
                verifier_instruction, plan_verifier_short_memory
            )
    ...
    def test_planner_and_verifier_cases_explicit_model(self) -> None:
        ...
        verifier_agent_context = AgentContext(
            agent_name="PlanVerifierAgent",
            agent_persona="Plan verifier agent.",
        )

        # Setup the Simple Agent
        planner_agent = PlannerAgentLogic(model=llm, context=planner_agent_context)
        verifier_agent = PlanVerifierAgentLogic(
            model=llm, context=verifier_agent_context
        )

        # Execute the agent
        planner_short_memory = ShortMemory()
        verifier_short_memory = ShortMemory()

        instruction_set = [
            # "What is the difference between BMW X3 and X4?",
            """Calculate the resistance of copper wire having a length 
            of 1 km and diameter 0.5 mm. (Resistivity of copper 1.7e-8Ωm)""",
            # "Calculate the volume of a sphere with radius 5 cm.",
            # """Does electric range of Tesla Model 3 allow for weekly
            # commute between San Francisco and Los Angeles?""",
        ]
        ...
        for instruction in instruction_set:
            result = planner_agent.run(instruction, planner_short_memory)
            ...
            verifier_result = verifier_agent.run(
                verifier_instruction, verifier_short_memory
            )
            console.print(verifier_result)
            verifier_plan = extract_tag_content(verifier_result, "JSON")
            verifier_parsed_plan = [json.loads(p) for p in verifier_plan]
